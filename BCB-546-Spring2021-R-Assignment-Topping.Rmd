...
``` {r}
fang_et_al_genotypes <- read.delim("~/Unix-Assignment-SP2021-Topping/Raw_Data/fang_et_al_genotypes.txt", header=FALSE)
snp_position.txt <- read.delim("~/Unix-Assignment-SP2021-Topping/Raw_Data/snp_position.txt", header=FALSE)
```
...
``` {r}
sapply(snp_position.txt, class)
```

For this data table, only `head()` and `tails()` were left out of the snp_position.txt analyses
