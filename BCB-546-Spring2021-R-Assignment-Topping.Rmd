...
snp_position %>% 
  ...
  drop_na() %>% 
  ggplot()+
  geom_bar(aes(x=Chromosome), color="red", fill="red")+ ggtitle("Number of SNPs per Chromosome (Maize and Teosinte)") + labs(x = "Chromosome", y = "Number of SNPs")
```
``` {r}
...
```
### Plot the distribution of SNPs along each chromosome by group based on Position values
``` {r warning=FALSE}
ggplot(data = merge_fang_pivot_numeric) + geom_bar(mapping = aes(as.numeric(Chromosome), fill=Group)) + scale_x_discrete(limit=c(1:10)) + labs(x = "Chromosome", y="SNPs Position by Group") + ggtitle("Distribution of SNPs along Chromosomes distributed by group") 
```

Add a new column to the `merge_fang_pivot` data frame called Zygosity

``` {r}
merge_fang_pivot$Zygosity <- "Heterozygous"
```

Replace all `"?/?"` with `Missing Data` in the Zygosity column

``` {r}
merge_fang_pivot$Zygosity[merge_fang_pivot$NT == "?/?"] <- "Missing Data"
```

For every "A/A", "C/C", "G/G", "T/T" site replace the value in the Zygosity column with `Homozygous`

``` {r}
merge_fang_pivot$Zygosity[merge_fang_pivot$NT %in% c("A/A", "C/C", "G/G", "T/T")] <- "Homozygous"
```

Plot the data of the proportion of heterozygous and homozygous sites, and missing data sites by Sample ID

``` {r warning=FALSE}
ggplot(data = merge_fang_pivot) + geom_bar(mapping=aes(x = Sample_ID, fill = Zygosity), position = "fill") + labs(x = "Sample_ID", y="Proportion") + ggtitle("Proportion of Heterozygous and Homozygous sites (with missing data)")
```
Plot the data of the proportion of heterozygous and homozygous sites, and missing data sites by group

``` {r warning=FALSE}
ggplot(data = merge_fang_pivot) + geom_bar(mapping = aes(x = Group, fill = Zygosity), position = "fill") + labs(x = "Group", y="Proportion") + ggtitle("Proportion of Heterozygous and Homozygous sites (with missing data) by group")
```                                                  
